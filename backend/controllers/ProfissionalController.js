...
exports.createUser = async (req, res) => {
    ...
    try {
        ...
        const verificaUsuarioCadastrado = await Profissional.findOne({ where: {nome_prof}, raw: true})

        if(verificaUsuarioCadastrado) return res.status(422).json({ msg: 'Nome de Usuario Já Existe' })
...
exports.loginUser = async (req, res) => {
    ...
    // Check User Exist
    try {
    const profissional = await Profissional.findOne({ where: {nome_prof}, raw: true})
    if(!profissional) return res.status(404).json({msg: 'Usuário Não Encontrado'})

    const checkPassword = await bcrypt.compareSync(senha_prof, profissional.senha_prof)
